...
class SolicitudEnvasadoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Obtener todos los envases disponibles (a trav�s de Inv_Envase)
...
def obtener_detalle_envase(request):
    """Obtener detalles de un envase espec�fico"""
...
def obtener_detalle_insumo(request):
    """Obtener detalles de un insumo espec�fico"""
...
def iniciar_envasado(request, pk):
    ...
    if solicitud.estado != 'Planificada':
        messages.error(request, 'Esta solicitud no est� preparada para iniciar.')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                # Verificar si ya se complet� toda la cantidad solicitada
